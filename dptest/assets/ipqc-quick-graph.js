...
(function(){
  ...
  function qgEnsurePlotElems(){
    // Default closer to the JMP screenshots used in this workflow:
    // first open should show raw data points + mean line + min/max box.
    ...
    if (st.box === undefined) st.box = true;

    // Legacy line-only default migration:
    // some flows still arrived here with {points:true,line:true,box:false} even on first open.
    // Only auto-flip once per dialog lifetime so later user toggles are respected.
    if (!QG._plotElemsDefaultMigrated){
      QG._plotElemsDefaultMigrated = true;
      if (st.points === true && st.line === true && st.box === false){
        st.box = true;
      }
    }
...
function qgGetLimitLabelAlign(colKey){
  const k = String(colKey || '');
  if (!QG.limitLabelAlignByCol) QG.limitLabelAlignByCol = {};
  return qgNormVarLabelAlign(QG.limitLabelAlignByCol[k]);
}

function qgSetLimitLabelAlign(colKey, align){
  const k = String(colKey || '');
  if (!k) return 'right';
  if (!QG.limitLabelAlignByCol) QG.limitLabelAlignByCol = {};
  const next = qgNormVarLabelAlign(align);
  QG.limitLabelAlignByCol[k] = next;
  return next;
}
...
// --- User-defined style dialog (JMP-like "사용자 정의...") ---
  ...
  function qgVarDashFromStyleKey(k){
    ...
    function liveApply(preserveValue){
      ...
      try{ syncVarLabelAlignInput(); }catch(e){}
...
function getBase(){
  ...
  function setPrimaryColKey(k){
    ...
    syncLimitLabelAlignInput();
    syncAxisInputs(true);
    syncOocSpecInputs(true);
    syncOocLineInputs(true);
    syncVarLabelAlignInput();
...
function qgRenderGroupYBox(){
  ...
  for (let si = 0; si < stripVars.length; si++){
    ...
    valStrip.setAttribute('data-varkey', String(varKey || ''));
    ...
    fldStrip.setAttribute('data-varkey', String(varKey || ''));
...
function qgPickGroupYBoxAt(x, y){
  ...
  let baseRect = r;
  try{
    const strips = qsa('.qg-group-y-strip[data-varkey]', el).filter(Boolean);
    if (strips.length){
      let bestVarKey = '';
      ...
      for (const strip of strips){
        ...
        if (inside){
          bestVarKey = String(strip.getAttribute('data-varkey') || '');
          bestDist = -1;
          break;
        }
      ...
      if (bestVarKey){
        const pair = strips.filter(node => String(node.getAttribute('data-varkey') || '') === bestVarKey);
        ...
        if (isFinite(left) && isFinite(top) && isFinite(right) && isFinite(bottom) && right > left && bottom > top){
          baseRect = { left, top, right, bottom, width:(right-left), height:(bottom-top) };
        }
      }
    }
  }catch(e){}
...
function qgShowVarDropMarker(target){
  ...
  const wholeRect = {
    ...
    width: Math.max(8, (baseRect.width || (baseRect.right - baseRect.left)) - inset * 2),
    height: Math.max(8, (baseRect.height || (baseRect.bottom - baseRect.top)) - inset * 2)
  };

  const left = Math.round(wholeRect.left);
  const top = Math.round(wholeRect.top);
...
// Initial sync for caption UI/button
    ...
    try{
      const limitAlignEl = qs('#qgLimitLabelAlign');
    ...
    try{
      const varAlignEl = qs('#qgVarLabelAlign');
      if (varAlignEl && !varAlignEl._qg){
        varAlignEl._qg = true;
        varAlignEl.addEventListener('change', ()=>{
          applyVarLabelAlignInputToState();
          renderLegend();
          renderGrid();
        });
      }
...
function renderFacetList(rootId, items, selSet){
  ...
  function syncLimitLabelAlignInput(){
    const el = qs('#qgLimitLabelAlign');
    if (!el) return;
    el.value = qgGetLimitLabelAlign(QG.sel.primaryColKey || '');
  }

  function applyLimitLabelAlignInputToState(){
    const k = String(QG.sel.primaryColKey || '');
    const el = qs('#qgLimitLabelAlign');
    if (!k || !el) return;
    qgSetLimitLabelAlign(k, el.value);
  }

  function syncVarLabelAlignInput(){
    const el = qs('#qgVarLabelAlign');
    if (!el) return;
    const st = qgEnsureUserStyle();
    el.value = qgNormVarLabelAlign(st && st.varline ? st.varline.labelAlign : 'right');
  }

  function applyVarLabelAlignInputToState(){
    const el = qs('#qgVarLabelAlign');
    if (!el) return;
    const st = qgEnsureUserStyle();
    if (st && st.varline) st.varline.labelAlign = qgNormVarLabelAlign(el.value);
    const dlgSel = qs('#qgUD_var_align');
    if (dlgSel) dlgSel.value = qgNormVarLabelAlign(el.value);
  }
...
function qgBuildTopHeaderSvg(toolsRow, cavs){
  ...
  if (xVars.length === 1){
    ...
    bandDrag(padL, y0, innerW, y2 - y0, xVar);
    return svg;
  }
...
function drawMatrixSvg(svg, tools, cavs, dates, opt){
  ...
  for (let pi=0; pi<nP; pi++){
    ...
    function hLine(val, label, cfg){
      ...
      const _align = qgGetLimitLabelAlign(_colKey);
      let _specX = _rowRight - 2;
      let _anchor = 'end';
      if (_align === 'left'){
        _specX = _rowLeft + 4;
        _anchor = 'start';
      }else if (_align === 'center'){
        _specX = _rowLeft + ((_rowRight - _rowLeft) / 2);
        _anchor = 'middle';
      }
      // Keep the label above the line and align it against the full graph width.
      const _labelGap = 4;
...
// per-FAI mean line style (line + dot)
    ...
    const drawRangeBox = (xPos, pointInfo, strokeColor, fillColor, dateInfo, dateKey, extraTip, styleOverride)=>{
      ...
      const snapStrokeCoord = (v)=>{
        const n = Number(v);
        if (!isFinite(n)) return 0;
        const w = Math.max(0.5, Number(useStrokeWidth) || 1);
        const roundedW = Math.round(w);
        const isOddInteger = Math.abs(w - roundedW) < 0.001 && (roundedW % 2 === 1);
        return Math.round(n) + (isOddInteger ? 0.5 : 0);
      };
      const xLeft = snapStrokeCoord(xPos - boxW/2);
      const xRight = snapStrokeCoord(xPos + boxW/2);
      const xMid = snapStrokeCoord(xPos);
      const boxTopPx = snapStrokeCoord(top);
      const boxBotPx = snapStrokeCoord(bot);
      const boxWpx = Math.max(1, xRight - xLeft);
      const boxHpx = Math.max(1, boxBotPx - boxTopPx);
      const rect = document.createElementNS(ns,'rect');
      rect.setAttribute('x', String(xLeft));
      rect.setAttribute('y', String(boxTopPx));
      rect.setAttribute('width', String(boxWpx));
      rect.setAttribute('height', String(boxHpx));
      ...
      if (hasBoxplot){
        const yMinV = snapStrokeCoord(yAt(pointInfo.min));
        const yMaxV = snapStrokeCoord(yAt(pointInfo.max));
        const yMedV = snapStrokeCoord(yAt(pointInfo.median));
        const capW = Math.max(12, boxW * 1.5);
        const capLeft = snapStrokeCoord(xPos - capW/2);
        const capRight = snapStrokeCoord(xPos + capW/2);
        const useStrokeOpacity = (styleOverride && styleOverride.boxOpacity !== undefined && styleOverride.boxOpacity !== null) ? qgClamp01(styleOverride.boxOpacity) : _boxOpacity;
        const useStrokeWidth = (styleOverride && styleOverride.boxStrokeWidth !== undefined && styleOverride.boxStrokeWidth !== null && isFinite(styleOverride.boxStrokeWidth)) ? Math.max(0.5, Number(styleOverride.boxStrokeWidth)) : _boxStrokeW;
        ...
        mkLine(xMid, boxTopPx, xMid, yMaxV);
        mkLine(xMid, boxBotPx, xMid, yMinV);
        mkLine(capLeft, yMaxV, capRight, yMaxV);
        mkLine(capLeft, yMinV, capRight, yMinV);
        mkLine(xLeft, yMedV, xRight, yMedV);
      }
    };
