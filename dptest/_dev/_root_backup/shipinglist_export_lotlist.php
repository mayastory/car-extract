...
// 우선순위: __DIR__/Templates  → __DIR__/templates → __DIR__ (기존 구조 호환)
// ─────────────────────────────────────────────────────────────
$TEMPLATES_ROOT = null;
$__tpl_base = __DIR__;
$__tpl_candidates = [
...
if (!is_dir($OQC_TEMPLATE_DIR)) { $OQC_TEMPLATE_DIR = __DIR__ . DIRECTORY_SEPARATOR . 'oqc templates'; }
if (!is_dir($OQC_TEMPLATE_DIR)) { $OQC_TEMPLATE_DIR = __DIR__ . DIRECTORY_SEPARATOR . 'OQC templates'; }
$OQC_PART_TPL = [
...
if (!is_dir($TEMPLATE_BASE_DIR)) { $TEMPLATE_BASE_DIR = __DIR__ . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'Memphis templates'; }
if (!is_dir($TEMPLATE_BASE_DIR)) { $TEMPLATE_BASE_DIR = __DIR__ . DIRECTORY_SEPARATOR . 'Templates' . DIRECTORY_SEPARATOR . 'Report Templates' . DIRECTORY_SEPARATOR . 'Memphis templates'; }
...
if (!function_exists('dp_delete_report_artifacts')) {
    function dp_delete_report_artifacts(int $reportId, bool $dry = false): array {
        $base = __DIR__ . DIRECTORY_SEPARATOR . 'exports' . DIRECTORY_SEPARATOR . 'reports';
...
// ─────────────────────────────────────────────────────────────
require_once __DIR__ . '/lib/xlsx_zip_patch.php';
require_once __DIR__ . '/lib/oqc_rowdata.php';
              ...
              <tr>
                ...
                <td>
                  ...
                  <?php else: ?>
                    ...
                    <?php $viewOk = is_dir(__DIR__ . '/exports/reports/rf_' . (int)($r['id'] ?? 0)); ?>
                  <?php if ($viewOk): ?>
...
$createdOqcFiles = [];
logline('  OQC 템플릿 기본폴더: ' . (__DIR__ . DIRECTORY_SEPARATOR . 'oqc templates'));
$oqcDiag = [];
...
foreach ($oqcAgg as $part => $agg) {
    ...
    $tpl = resolve_oqc_template_path($part, $mapped, __DIR__);
    if (!$tpl || !is_file($tpl)) {
