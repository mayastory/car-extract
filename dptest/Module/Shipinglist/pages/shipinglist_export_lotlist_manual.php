...
//  - 관리자 + ?debug=1 인 경우에만 화면에 에러를 노출(기존 디버깅 유지)
//  - 그 외에는 error_log 로만 남김
// ─────────────────────────────
$IS_ADMIN = !empty($_SESSION['dp_admin_id']);
$GLOBALS['IS_ADMIN'] = $IS_ADMIN ? 1 : 0;

// 디버그 화면 노출은 관리자 + debug=1만 허용
$ALLOW_VERBOSE_ERRORS = ($IS_ADMIN && ((int)($_GET['debug'] ?? 0) === 1));
if (!$ALLOW_VERBOSE_ERRORS) {
    @ini_set('display_errors', '0');
    @ini_set('html_errors', '0');
}

@ini_set('log_errors', '1');
$__logDir = JTMES_ROOT . DIRECTORY_SEPARATOR . 'exports' . DIRECTORY_SEPARATOR . 'logs';
if (!is_dir($__logDir)) { @mkdir($__logDir, 0777, true); }
@ini_set('error_log', $__logDir . DIRECTORY_SEPARATOR . 'shipinglist_export_lotlist_error.log');

if (!$ALLOW_VERBOSE_ERRORS) {
    // warning/notice 등을 화면에 찍지 않고 로그로만 남김
    set_error_handler(function(int $severity, string $message, string $file = '', int $line = 0) {
        // suppressed (@)는 무시
        if (!(error_reporting() & $severity)) return true;
        error_log("[PHP] {$message} @ {$file}:{$line}");
        return true;
    });
...
// (고정) 본 성적서/ZIP 생성은 Memphis 5종만 대상으로 처리한다.
// - ShipingList에 다른 프로젝트/품번이 섞여 있어도 포함하지 않음.
// - 특정 모델이 출하되지 않은 날은 자동으로 누락(0건) 처리된다.
$SHIP_PART_PARAMS = [];
$SHIP_PART_IN_SQL = '';
{
    $shipParts = array_keys($PART_MAP); // 5종
    $placeholders = [];
    foreach ($shipParts as $i => $p) {
        $ph = ':ship_part_' . $i;
        $placeholders[] = $ph;
        $SHIP_PART_PARAMS[$ph] = $p;
    }
    $SHIP_PART_IN_SQL = implode(',', $placeholders);
    unset($shipParts, $placeholders, $i, $p, $ph);
}
...
function oqc_build_selected_tc_set_from_slots(array $toolPairs, array $headerIds = [], bool $requireHeader = true): array {
    $out = [];
    $n = count($toolPairs);
    for ($i = 0; $i < $n; $i++) {
        if ($requireHeader) {
...
function oqc_build_missing_tc_set_from_ship_and_slots(array $shipPairsAll, array $toolPairs, array $headerIds = [], bool $requireHeader = true): array {
    $shipSet = [];
    foreach (array_keys($shipPairsAll) as $tcRaw) {
        $norm = normalize_tool_cavity_key((string)$tcRaw);
        if ($norm === '' || $norm === null) continue;
        $shipSet[$norm] = true;
    }
    if (empty($shipSet)) return [];

    $selSet = oqc_build_selected_tc_set_from_slots($toolPairs, $headerIds, $requireHeader);
    foreach (array_keys($selSet) as $norm) {
        unset($shipSet[$norm]);
    }
    return $shipSet;
}

function oqc_format_missing_tc_summary_text(array $missingTcSet): string {
    if (empty($missingTcSet)) return '없음';

    $toolCavs = [];
    foreach (array_keys($missingTcSet) as $tc) {
        $p = parse_tool_cavity_pair((string)$tc);
        if (!$p) continue;
        [$tool, $cav] = $p;
        if (!isset($toolCavs[$tool])) $toolCavs[$tool] = [];
        $toolCavs[$tool][(int)$cav] = true;
    }
    if (empty($toolCavs)) return '없음';
    ...
    foreach ($tools as $tool) {
        ...
        if (!$cavs) continue;
        $lines[] = $tool . ': ' . implode(',', $cavs) . ' /Cav';
    }
    return $lines ? implode("\n", $lines) : '없음';
}

function oqc_build_row_cell_refs(string $startCol, string $endCol, int $rowNum): array {
    $out = [];
    $a = function_exists('xlsx_col_to_index') ? xlsx_col_to_index($startCol) : null;
    $b = function_exists('xlsx_col_to_index') ? xlsx_col_to_index($endCol) : null;
    if (!$a || !$b || $a < 1 || $b < $a) return $out;
    for ($i = $a; $i <= $b; $i++) {
        $col = function_exists('xlsx_index_to_col') ? xlsx_index_to_col($i) : '';
        if ($col === '') continue;
        $out[] = $col . $rowNum;
    }
    return $out;
...
/**
 * OQC 최종 파일에서 사용자 확인용 셀(예: AK2)을 "보이는 텍스트"로 강제 기록하고,
 * 동적 배열 spill 잔여값(예: AL2:BP2)을 비워서 표시/재계산 충돌을 줄인다.
 * 구조 손상 방지를 위해 시트 XML 값만 최소 수정한다.
 */
function xlsx_oqc_force_visible_summary_cells_zip(string $xlsxPath, array $textCellsByRef, array $clearCellRefs = [], string $sheetName = 'OQC'): bool {
    if (!is_file($xlsxPath)) return false;
    ...
    try {
        ...
        if (!$sheetPath) { $zip->close(); return false; }

        $xml = $zip->getFromName($sheetPath);
        if ($xml === false || $xml === '') { $zip->close(); return false; }

        $dom = new DOMDocument();
        $dom->preserveWhiteSpace = true;
        $dom->formatOutput = false;
        if (!@ $dom->loadXML($xml)) { $zip->close(); return false; }

        $xp = new DOMXPath($dom);
        $xp->registerNamespace('x', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main');

        $stripCell = static function(DOMElement $c): void {
            $toRemove = [];
            foreach ($c->childNodes as $ch) {
                if ($ch instanceof DOMElement) {
                    $ln = $ch->localName;
                    if ($ln === 'f' || $ln === 'v' || $ln === 'is') $toRemove[] = $ch;
                }
            }
        ...
        foreach ($clearCellRefs as $ref) {
            $ref = strtoupper(trim((string)$ref));
            if ($ref === '') continue;
            $c = $xp->query('//x:sheetData//x:c[@r="' . $ref . '"]')->item(0);
            if (!($c instanceof DOMElement)) continue;
            $stripCell($c);
        }

        foreach ($textCellsByRef as $ref => $value) {
            $ref = strtoupper(trim((string)$ref));
            if ($ref === '' || !preg_match('/^([A-Z]+)(\d+)$/', $ref, $m)) continue;
            $rowNum = (int)$m[2];

            $rowNode = xlsx_ensure_row($dom, $xp, $rowNum);
            $c = $xp->query('x:c[@r="' . $ref . '"]', $rowNode)->item(0);
            if (!($c instanceof DOMElement)) {
                $styleS = null;
                $probe = $xp->query('x:c[starts-with(@r, "AK")]', $rowNode)->item(0);
                if ($probe instanceof DOMElement && $probe->hasAttribute('s')) $styleS = $probe->getAttribute('s');
                $c = xlsx_ensure_cell($dom, $xp, $rowNode, $ref, $styleS);
            }
            $stripCell($c); // formula 포함 제거 → 최종 표시값 보장
            xlsx_set_cell_value($dom, $c, (string)$value);
        }
...
if ($action === 'fetch') {
    ...
    // consume=0: 파일 유지(관리자/특수 상황에서 재다운로드용)
...
$stmt = $pdo->query("SELECT DISTINCT ship_to FROM ShipingList WHERE ship_to <> '' ORDER BY ship_to");
$shipToList = $stmt ? $stmt->fetchAll(PDO::FETCH_COLUMN) : [];
$selectedShipTo = $_REQUEST['ship_to'] ?? ($shipToList[0] ?? '');
if (empty($shipToList)) {
    $shipToList = [''];
    $selectedShipTo = '';
}
...
$manualSpecDefault = [
    'IR' => (string)($_REQUEST['manual_spec']['IR'] ?? ''),
    'XC' => (string)($_REQUEST['manual_spec']['XC'] ?? ''),
    'YC' => (string)($_REQUEST['manual_spec']['YC'] ?? ''),
    'ZC' => (string)($_REQUEST['manual_spec']['ZC'] ?? ''),
    'ZS' => (string)($_REQUEST['manual_spec']['ZS'] ?? ''),
];
      ...
      <div class="row">
        ...
        <div class="col">
          ...
          <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
            <button type="submit" class="btn btn-primary">수동성적서 생성</button>
          </div>
        </div>
      </div>

      <style>
        .manual-tabbar{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;margin-bottom:10px;}
        .manual-tab{height:32px;padding:0 12px;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);color:var(--fg);cursor:pointer;font-weight:700;}
        .manual-tab.active{background:rgba(79,140,255,.20);border-color:rgba(138,180,248,.55);color:#fff;}
        .manual-pane{display:none;}
        .manual-pane.active{display:block;}
        .manual-spec{width:100%;min-height:260px;resize:vertical;padding:12px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:var(--bg);color:var(--fg);font:13px/1.55 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;box-sizing:border-box;}
      </style>
      <div class="desc" style="margin-bottom:10px;">생산일은 <b>2026-03-21</b>, <b>20260321</b>, <b>260321</b> 형식 모두 인식합니다. Tool/Cavity는 <b>A: 3,4</b> 형식으로 입력하세요.</div>
      <div class="manual-tabbar">
        <?php foreach (['IR','XC','YC','ZC','ZS'] as $i => $code): ?>
          <button type="button" class="manual-tab <?= $i===0 ? 'active' : '' ?>" data-tab="<?=h($code)?>"><?=h($code)?></button>
        <?php endforeach; ?>
      </div>
      <?php foreach (['IR','XC','YC','ZC','ZS'] as $i => $code): ?>
        <div class="manual-pane <?= $i===0 ? 'active' : '' ?>" data-pane="<?=h($code)?>">
          <label><?=h($code)?> 수동 입력</label>
          <textarea class="manual-spec" name="manual_spec[<?=h($code)?>]" spellcheck="false"><?= h((string)($manualSpecDefault[$code] ?? '')) ?></textarea>
        </div>
      <?php endforeach; ?>
    </form>
<script>
(function(){
  document.querySelectorAll('.manual-tab').forEach(function(btn){
    btn.addEventListener('click', function(){
      var code = btn.getAttribute('data-tab');
      document.querySelectorAll('.manual-tab').forEach(function(x){ x.classList.remove('active'); });
      document.querySelectorAll('.manual-pane').forEach(function(x){ x.classList.remove('active'); });
      btn.classList.add('active');
      var pane = document.querySelector('.manual-pane[data-pane="' + code + '"]');
...
$manualSpecInput = (isset($_REQUEST['manual_spec']) && is_array($_REQUEST['manual_spec'])) ? $_REQUEST['manual_spec'] : [];
$manualSpecsByPart = [];
$manualErrors = [];
foreach (['IR','XC','YC','ZC','ZS'] as $shortCode) {
    $fullPart = manual_short_part_to_full($shortCode);
    if (!$fullPart) continue;
    $parsed = parse_manual_report_spec((string)($manualSpecInput[$shortCode] ?? ''));
    if (!empty(trim((string)($manualSpecInput[$shortCode] ?? '')))) {
        foreach (($parsed['errors'] ?? []) as $err) { $manualErrors[] = '[' . $shortCode . '] ' . $err; }
        if (!empty($parsed['dates']) && !empty($parsed['tool_map'])) {
            $manualSpecsByPart[$fullPart] = $parsed;
        } elseif (empty($parsed['errors'])) {
            $manualErrors[] = '[' . $shortCode . '] 생산일 또는 Tool/Cavity 정보가 부족합니다.';
        }
    }
}
...
if ($fromDate === '' || $toDate === '' || $shipTo === '' || !empty($manualErrors) || empty($manualSpecsByPart)) {
    $msg = '';
    if ($shipTo === '') {
        $msg = '납품처를 선택해 주세요.';
    } elseif ($fromDate === '' || $toDate === '') {
        $msg = '조회기간(from_date / to_date)을 확인해 주세요.';
    } elseif (!empty($manualErrors)) {
        $msg = implode(' / ', $manualErrors);
    } else {
        $msg = '수동 입력된 생산일 / Tool#Cavity가 없습니다.';
    }
    ...
    if (isset($_REQUEST['hist_month'])) $params['hist_month'] = (string)$_REQUEST['hist_month'];
    if (isset($_REQUEST['embed']))     $params['embed']     = (string)$_REQUEST['embed'];
    foreach (['IR','XC','YC','ZC','ZS'] as $shortCode) {
        $params['manual_spec'][$shortCode] = (string)($manualSpecInput[$shortCode] ?? '');
    }

    $self = $_SERVER['PHP_SELF'] ?? 'shipinglist_export_lotlist_manual.php';
...
<div class="wrap">
  ...
  <div class="card">
    <div class="small">기간: <?=h($fromDate)?> ~ <?=h($toDate)?> / 납품처: <?=h($shipTo)?> / 방식: 수동 입력</div>
...
function build_abort(string $msg, ?Throwable $e = null): void {
    // 화면에는 최소 메시지만(경로/스택트레이스 숨김)
    logline($msg, 'ERROR');
    if (!empty($GLOBALS['DEBUG']) && $e) {
        logline('[DEBUG] ' . $e->getMessage(), 'DEBUG');
    }
    // 남은 HTML 닫기(로그 영역/카드/랩)
    echo "</div></div></div>";
    echo "</body></html>";
    exit;
}
...
function oqc_gc_temp_exports(int $ttlSec = 86400): void {
    $tmp = (string)sys_get_temp_dir();
    if ($tmp === '' || !is_dir($tmp)) return;

    $now = time();
    $cut = $now - max(60, $ttlSec);

    // 1) meta 기반 정리
    foreach (glob($tmp . DIRECTORY_SEPARATOR . 'oqc_export_meta_*.json') ?: [] as $metaPath) {
        if (!is_file($metaPath)) continue;
        $mt = @filemtime($metaPath);
        if ($mt !== false && $mt > $cut) continue;
        ...
        if (is_array($meta)) {
            ...
            if ($workDir !== '' && is_dir($workDir)) { rrmdir_safe($workDir); }
        }
        @unlink($metaPath);
    }

    // 2) meta 없이 남은 orphan workdir 정리
    foreach (glob($tmp . DIRECTORY_SEPARATOR . 'oqc_export_*') ?: [] as $dir) {
        if (!is_dir($dir)) continue;
        $base = basename($dir);
        if (!preg_match('/^oqc_export_[a-f0-9]{32}$/', $base)) continue;
...
if (!function_exists('xlsx_restore_oqc_spill_formulas_and_force_recalc_zip')) {
function xlsx_restore_oqc_spill_formulas_and_force_recalc_zip($templatePath, $outPath, $sheetXmlPath = 'xl/worksheets/sheet1.xml') {
    if (!is_file($outPath)) return false;
    $zt = new ZipArchive();
    $zo = new ZipArchive();
    $tplOpened = (is_file($templatePath) && ($zt->open($templatePath) === true));
    if ($zo->open($outPath) !== true) { if ($tplOpened) $zt->close(); return false; }

    $tplSheet = ($tplOpened ? $zt->getFromName($sheetXmlPath) : false);
    $outSheet = $zo->getFromName($sheetXmlPath);

    if ($tplSheet !== false && $outSheet !== false) {
        // OQC 템플릿의 동적 배열 앵커(실제 표시셀)를 복원: AK2 / AK4 / AK5
        // (구버전 템플릿 호환용으로 B2/B4/B5도 함께 시도)
        $targets = ['AK2','AK4','AK5','B2','B4','B5'];
        foreach ($targets as $ref) {
            if (preg_match('~(<c\b[^>]*\br="' . preg_quote($ref, '~') . '"[^>]*>.*?</c>)~s', $tplSheet, $mTpl)) {
                $tplCell = $mTpl[1];
                if (preg_match('~<c\b[^>]*\br="' . preg_quote($ref, '~') . '"[^>]*>.*?</c>~s', $outSheet)) {
                    $outSheet = preg_replace('~<c\b[^>]*\br="' . preg_quote($ref, '~') . '"[^>]*>.*?</c>~s', $tplCell, $outSheet, 1);
                } else {
                    // Insert into the corresponding row if missing (best effort).
                    $rowNum = intval(preg_replace('/\D+/', '', $ref));
                    if (preg_match('~(<row\b[^>]*\br="' . $rowNum . '"[^>]*>)(.*?)(</row>)~s', $outSheet, $mr)) {
                        $newRow = $mr[1] . $mr[2] . $tplCell . $mr[3];
                        $outSheet = preg_replace('~<row\b[^>]*\br="' . $rowNum . '"[^>]*>.*?</row>~s', $newRow, $outSheet, 1);
                    }
                }
            }
        }
        // ZipArchive에는 addFromName()이 없습니다. (addFromString/addFile만 존재)
        // 여기서는 문자열(XML)을 덮어써야 하므로 addFromString()을 사용합니다.
        $zo->addFromString($sheetXmlPath, $outSheet);
    }

    // Force Excel recalc on open (important for spilled formula outputs like AK2/AK4/AK5).
    $wbPath = 'xl/workbook.xml';
    $wb = $zo->getFromName($wbPath);
    if ($wb !== false) {
        if (preg_match('~<calcPr\b[^>]*/>~s', $wb)) {
            $wb = preg_replace_callback('~<calcPr\b([^>]*)/>~s', function($m){
                $attrs = $m[1];
                $attrs = preg_replace('~\s(calcMode|fullCalcOnLoad|forceFullCalc|calcOnSave)="[^"]*"~', '', $attrs);
                return '<calcPr' . $attrs . ' calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1" calcOnSave="1"/>';
            }, $wb, 1);
        } elseif (preg_match('~<calcPr\b[^>]*>.*?</calcPr>~s', $wb)) {
            $wb = preg_replace_callback('~<calcPr\b([^>]*)>(.*?)</calcPr>~s', function($m){
                $attrs = $m[1];
                $attrs = preg_replace('~\s(calcMode|fullCalcOnLoad|forceFullCalc|calcOnSave)="[^"]*"~', '', $attrs);
                return '<calcPr' . $attrs . ' calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1" calcOnSave="1"></calcPr>';
            }, $wb, 1);
        } else {
            $wb = preg_replace('~</workbook>~', '<calcPr calcId="0" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1" calcOnSave="1"/></workbook>', $wb, 1);
        }
        $zo->addFromString($wbPath, $wb);
    }

    // Remove calcChain so Excel rebuilds dependency graph.
    $calcChainPath = 'xl/calcChain.xml';
    if ($zo->locateName($calcChainPath) !== false) {
        @$zo->deleteName($calcChainPath);
    }
    $ctPath = '[Content_Types].xml';
    $ct = $zo->getFromName($ctPath);
    if ($ct !== false) {
        $ct2 = preg_replace('~<Override[^>]*PartName="/xl/calcChain\.xml"[^>]*/>\s*~s', '', $ct);
        if ($ct2 !== null) $zo->addFromString($ctPath, $ct2);
    }
    $relsPath = 'xl/_rels/workbook.xml.rels';
    $rels = $zo->getFromName($relsPath);
    if ($rels !== false) {
        $rels2 = preg_replace('~<Relationship\b[^>]*Type="[^"]*/calcChain"[^>]*/>\s*~s', '', $rels);
        if ($rels2 !== null) $zo->addFromString($relsPath, $rels2);
    }

    $zo->close();
    if ($tplOpened) $zt->close();
    return true;
}}
...
// ============================================================================
// Excel Recalc Helper (for spilled formulas like AK2/AK4/AK5)
// - Some environments require forcing calcPr flags in workbook.xml
// ============================================================================
if (!function_exists('sl_force_xlsx_full_recalc')) {
    function sl_force_xlsx_full_recalc($xlsxPath) {
        if (!is_file($xlsxPath)) return false;
        if (!class_exists('ZipArchive')) return false;

        $zo = new ZipArchive();
        if ($zo->open($xlsxPath) !== true) return false;
...
logline('1) 수동 입력 데이터 정리 중...');

[$manualRows, $counts] = manual_build_rows_from_specs($manualSpecsByPart, $toDate);
$sumQty = [];
foreach (array_keys($counts) as $p) { $sumQty[$p] = 0; }
if (!$counts || !$manualRows) {
    logline('수동 입력 데이터 없음. 종료.');
...
// 작업 디렉터리/토큰
//  - temp에 남는 찌꺼기(meta/workdir)를 주기적으로 정리(기본 24h)
//  - 토큰/폴더명 충돌(동일 이름 폴더가 이미 존재) 방지
oqc_gc_temp_exports(86400); // 24시간

do {
    $token = bin2hex(random_bytes(16));
    $workDir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . "oqc_export_{$token}";
    $metaProbe = sys_get_temp_dir() . DIRECTORY_SEPARATOR . "oqc_export_meta_{$token}.json";
} while (is_dir($workDir) || is_file($metaProbe));

if (!@mkdir($workDir, 0777, true) && !is_dir($workDir)) {
    build_abort('오류: 임시 작업 폴더를 만들 수 없습니다. (권한/경로 확인)');
}

// OQC 생성 파일은 workDir/OQC 아래에 저장 (대문자 폴더명 고정)
$oqcDir = $workDir . DIRECTORY_SEPARATOR . 'OQC';
if (!@mkdir($oqcDir, 0777, true) && !is_dir($oqcDir)) {
    build_abort('오류: 임시 OQC 폴더를 만들 수 없습니다. (권한/경로 확인)');
}

// CMM(자화용) 템플릿은 workDir/cmm 아래에 저장
$cmmDir = $workDir . DIRECTORY_SEPARATOR . 'cmm';
if (!@mkdir($cmmDir, 0777, true) && !is_dir($cmmDir)) {
    build_abort('오류: 임시 CMM 폴더를 만들 수 없습니다. (권한/경로 확인)');
}
// 디버그용: 실제 임시 경로/작업 경로를 화면 로그에 출력
// 사용 방법: build 호출 시 ?debug_tmp=1 추가
if (!empty($ALLOW_VERBOSE_ERRORS) && ((string)($_REQUEST['debug_tmp'] ?? '') === '1')) {
    logline('TMPDIR: ' . sys_get_temp_dir());
...
$stmtRows = $manualRows;
$stmtIndex = 0;
$nextManualRow = function() use (&$stmtRows, &$stmtIndex) {
    if (!isset($stmtRows[$stmtIndex])) return false;
    return $stmtRows[$stmtIndex++];
};
...
// 남은 chunk flush
    ...
    if (!is_dir($workDir)) { build_abort('오류: 임시 작업 폴더가 없습니다(중간 삭제됨). 다시 발행해 주세요.'); }
    ...
    try {
        ...
        // 중간에 임시폴더가 삭제된 경우(사용자가 temp를 정리하거나, 외부 정리툴 등) 1회 복구 시도
        if (!is_dir($workDir)) { @mkdir($workDir, 0777, true); }
        if (is_dir($workDir)) {
            try { $writer->save($tmpPath); }
            catch (Throwable $e2) { build_abort('오류: REPORT 파일 저장 실패', $e2); }
        } else {
            build_abort('오류: 임시 작업 폴더가 없어 REPORT 저장 불가', $e);
        }
    }
...
$usedHeaderIds = []; // 이번 실행에서 이미 선택한 OQC header id (중복 선택 방지)
    ...
    $buildSelectedTcSet = function(bool $requireHeader = true) use (&$toolPairs, &$headerIds): array {
        return oqc_build_selected_tc_set_from_slots($toolPairs, $headerIds, $requireHeader);
    };
...
// PASS-2) 남은 칸은 (생산일 → 툴 → 캐비티) 라운드로빈으로, DB 있는 것만 추가
    $pass2DeferredDup = []; // [prodDate, tcNorm] soft 다양성 우선으로 보류된 PASS2 중복 TC
...
foreach ($dateList as $prodDate) {
        ...
        for ($depth = 0; $depth < $maxDepth; $depth++) {
            foreach ($tools as $tool) {
                ...
                // soft 우선: 이미 선택된 Tool#Cavity는 PASS2에서 반복 채우지 않고,
                // 출하 기준으로 아직 비어있는 Tool#Cavity를 먼저 소진한다.
                $selectedTcSetNow = $buildSelectedTcSet(true);
                if (isset($selectedTcSetNow[$tcNorm])) {
                    // PASS2 1차는 다양성 우선: 같은 Tool#Cavity 재사용 후보는 보류해두고,
                    // PASS2 종료 직전에 빈칸이 남으면 PASS2-2에서 다시 사용한다.
                    $pass2DeferredDup[] = [$prodDate, $tcNorm];
                    continue;
                }
    ...
    // PASS2-2) 1차 다양성 우선으로 보류된 "중복 tc" 후보를 마지막에 소프트 보강
    //  - PASS2에서 채울 수 있는 것이 충분한데도 하드 중복 금지처럼 동작해 PASS3로 넘어가는 상황 방지
    if ($colIdx < $maxCols && !empty($pass2DeferredDup)) {
        $seenDupKeys = [];
        foreach ($pass2DeferredDup as $defer) {
            if ($colIdx >= $maxCols) break;
            $prodDate = (string)($defer[0] ?? '');
            $tcNorm   = (string)($defer[1] ?? '');
            if ($prodDate === '' || $tcNorm === '') continue;

            $dk = $prodDate . '|' . $tcNorm;
            if (isset($seenDupKeys[$dk])) continue;
            $seenDupKeys[$dk] = true;
...
// ─────────────────────────────────────────────
        ...
        if (false && !empty($reservedTCs) && $reservedFaiCols < 32) {
            ...
            // 3) 빈 칸 재채움: 출하 기반 missing Tool#Cavity만, 중복 TC 금지
            //    - shipMissingSet 후보가 없으면 강제 재채움하지 않음(빈칸 허용)
            ...
            if ($fillPos < 32 && !empty($shipMissingSet)) {
                $seenTc = [];
                foreach ($toolPairs as $tc0) {
                    $k0 = normalize_tool_cavity_key((string)$tc0);
                    if ($k0 !== '') $seenTc[$k0] = true;
                }
                ...
                foreach ($dateList as $prodDate) {
                    ...
                    foreach ($pairs as $tcRaw) {
                        ...
                        $tcNorm = normalize_tool_cavity_key($tcRaw);
                        if (!$tcNorm) continue;
                        if (!isset($shipMissingSet[$tcNorm])) continue; // 출하 기반 missing 대상만
                        if (isset($reservedTCs[$tcNorm])) continue;
                        if (isset($seenTc[$tcNorm])) continue;          // 중복 TC 금지
...
logline("  - 총 채움: {$filledCnt}/32");
    ...
    $oqcAk2MissingText = oqc_format_missing_tc_summary_text(
        oqc_build_missing_tc_set_from_ship_and_slots($shipPairsAll, $toolPairs, $headerIds, true)
    );
...
$outName = "OQC_{$part}.xlsx";
    if (!is_dir($oqcDir)) { @mkdir($oqcDir, 0777, true); }
    if (!is_dir($oqcDir)) { build_abort('오류: 임시 OQC 폴더가 없습니다(중간 삭제됨). 다시 발행해 주세요.'); }
    ...
    try {
        ...
        @xlsx_restore_oqc_spill_formulas_and_force_recalc_zip($tplPath, $outPath, 'xl/worksheets/sheet1.xml');
        if (function_exists('sl_force_xlsx_full_recalc')) { @sl_force_xlsx_full_recalc($outPath); }
        // AK2는 최종 파일에서 보이는 값으로 한 번 더 강제 기록(동적 배열 재계산 실패 대비)
        // + AK2/AK4/AK5 spill 영역 잔여값 정리(앵커 제외)
        $spillClearRefs = array_merge(
            oqc_build_row_cell_refs('AL', 'BP', 2),
            oqc_build_row_cell_refs('AL', 'BP', 4),
            oqc_build_row_cell_refs('AL', 'BP', 5)
        );
        @xlsx_oqc_force_visible_summary_cells_zip(
            $outPath,
            ['AK2' => (string)($oqcAk2MissingText ?? '없음')],
            $spillClearRefs,
            'OQC'
        );
        ...
        if ($IS_JAWHA) {
            if (!is_dir($cmmDir)) { @mkdir($cmmDir, 0777, true); }
            if (!is_dir($cmmDir)) { build_abort('오류: 임시 CMM 폴더가 없습니다(중간 삭제됨). 다시 발행해 주세요.'); }
...
// ─────────────────────────────
try {
    // parts_json: 수동 발행은 품번 목록만 저장(출하수량은 ShipingList 미사용)
    $partsUniq = array_keys($counts);
    sort($partsUniq);

    $partsPayload = ['parts' => array_values($partsUniq)];
...
setTimeout(() => {
  (window.top || window).location.href = '?action=fetch&token=<?=h($token)?>&consume=1';
}, 800);
</script>
</body>
</html>
