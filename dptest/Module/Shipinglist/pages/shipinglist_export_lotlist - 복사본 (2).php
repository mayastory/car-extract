...
$GLOBALS['DEBUG'] = $DEBUG;
// ─────────────────────────────
// REPORT(성적서) FAI/SPC 시트의 "측정값 32칸( Tool#Cavity )" 시작 열 설정
//  - 원래(기존) 동작 = LGIT 템플릿 기준 위치로 채워짐
//  - 자화(JAWHA) 템플릿은 열 위치가 다를 수 있어, 여기서 "시작 열"만 쉽게 교정할 수 있게 함
//  - ⚠️ enabled=false 기본: 기존 동작 100% 동일 (아무 변화 없음)
//  - enabled=true 이고, (src != dst) 일 때만,
//      patch_oqc_report_from_rowdata() 실행 이후 값만 "복사"해서 dst 열에도 한 번 더 써준다.
//      (기존 src 위치 값은 지우지 않음: 안전 우선)
//  - 모델별로 열이 다르면 by_model에 part_name 키로 override
// ─────────────────────────────
$GLOBALS['REPORT_FAI_SPC_COLCFG'] = $GLOBALS['REPORT_FAI_SPC_COLCFG'] ?? [
    'enabled' => false,

    // ✅ "원래(기존) 템플릿"에서 값이 찍히는 시작 열(=LGIT 템플릿 기준)
    //   - FAI: 샘플 3칸 (예: L,M,N)
    //   - SPC: 측정 32칸 (예: AM..BR)
    'src' => [
        'FAI' => 'L',
    ...
    // ✅ 납품처(템플릿 폴더)별로 "원하는" 시작 열
    //   - LGIT는 보통 src와 동일
    //   - JAWHA는 템플릿에 맞게 변경
    'dst' => [
        // LGIT 템플릿 (대부분 모델: FAI=L..N, SPC=AM..BR)
        'LGIT'  => ['FAI' => 'L', 'SPC' => 'AM'],

        // JAWHA 템플릿 (대부분 모델: FAI=H..J, SPC=AI..BN)
        'JAWHA' => ['FAI' => 'H', 'SPC' => 'AI'],
    ],

    // ✅ 모델(part_name)별 override (템플릿 구조가 다른 경우)
    // - MEM-Z-STOPPER (LGIT 템플릿만 FAI/SPC 시작열이 다름)
    'by_model' => [
        'MEM-Z-STOPPER' => [
            // LGIT Z-STOPPER: FAI=J..L, SPC=AK..BP
            'src' => ['FAI' => 'J', 'SPC' => 'AK'],
            'dst' => [
                'LGIT'  => ['FAI' => 'J', 'SPC' => 'AK'],
                'JAWHA' => ['FAI' => 'H', 'SPC' => 'AI'],
...
// ─────────────────────────────
// REPORT(성적서) FAI/SPC 시트 열 교정 유틸 (XLSX ZIP XML 직접 패치)
//  - PhpSpreadsheet로 load/save 하면 도형/서식/정의이름 등이 흔들릴 수 있어, 값만 ZIP 내부 XML로 패치한다.
// ─────────────────────────────
function report_excel_col_to_num(string $col): int {
    if (function_exists('cmm_excel_col_to_num')) return cmm_excel_col_to_num($col);
    $col = strtoupper(trim($col));
    $n = 0;
    for ($i = 0; $i < strlen($col); $i++) {
        $ch = ord($col[$i]);
...
function report_excel_num_to_col(int $n): string {
    if (function_exists('cmm_excel_num_to_col')) return cmm_excel_num_to_col($n);
    $s = '';
    while ($n > 0) {
        $r = ($n - 1) % 26;
        $s = chr(65 + $r) . $s;
...
function report_build_col_list(string $startCol, int $count): array {
    $start = report_excel_col_to_num($startCol);
    if ($start <= 0) return [];
    $out = [];
    for ($i = 0; $i < $count; $i++) {
        $out[] = report_excel_num_to_col($start + $i);
    }
    return $out;
}

// workbook.xml + rels 를 읽어서 sheet name(키워드 포함) → worksheet xml 경로를 찾는다.
function report_xlsx_find_sheet_xml_by_keyword(ZipArchive $zip, string $keyword): ?string {
    ...
    // rId -> target
    $ridToTarget = [];
    if (preg_match_all('/<Relationship[^>]+Id="([^"]+)"[^>]+Target="([^"]+)"[^>]*\/>/i', $rels, $mm, PREG_SET_ORDER)) {
        foreach ($mm as $m) {
            $ridToTarget[$m[1]] = $m[2];
        }
    }
    ...
    // sheet name + r:id
    if (!preg_match_all('/<sheet\b[^>]*name="([^"]+)"[^>]*r:id="([^"]+)"[^>]*\/>/i', $wb, $mm, PREG_SET_ORDER)) {
        return null;
    }
    ...
    foreach ($mm as $m) {
        $name = (string)$m[1];
        $rid  = (string)$m[2];
        if (stripos($name, $keyword) === false) continue;

        $t = $ridToTarget[$rid] ?? null;
        if (!$t) continue;

        $t = ltrim($t, '/');
        if (stripos($t, 'worksheets/') !== false) {
            $path = 'xl/' . $t;
            if ($zip->locateName($path) !== false) return $path;
        } else {
            // 혹시 상대경로가 다르면 best-effort
            $path = 'xl/' . $t;
            if ($zip->locateName($path) !== false) return $path;
            $path = 'xl/worksheets/' . basename($t);
            if ($zip->locateName($path) !== false) return $path;
        }
    }
...
// sheet xml 안에서 srcStartCol(32칸)을 읽어서 dstStartCol(32칸)으로 "값만" 복사
function report_xlsx_copy_toolcols_in_sheet_xml(string $sheetXml, string $srcStartCol, string $dstStartCol, int $colCount = 32): array {
    $srcCols = report_build_col_list($srcStartCol, $colCount);
    $dstCols = report_build_col_list($dstStartCol, $colCount);
    if (!$srcCols || !$dstCols || count($srcCols) !== count($dstCols)) {
        return ['xml' => $sheetXml, 'copied' => 0];
    }

    $copied = 0;

    // row 단위로만 patch (성능/안정)
    $sheetXml2 = preg_replace_callback('/<row\b([^>]*)>(.*?)<\/row>/s', function($m) use ($srcCols, $dstCols, &$copied) {
        $rowAttr = $m[1];
        $rowInner = $m[2];

        if (!preg_match('/\br="(\d+)"/', $rowAttr, $rm)) {
            return $m[0];
        }
        $r = (int)$rm[1];
        if ($r <= 0) return $m[0];

        // 각 칼럼별로 src cell이 "값/수식"을 가지고 있으면 dst cell에 내용을 덮어쓴다(스타일은 dst 유지)
        for ($i = 0; $i < count($srcCols); $i++) {
            $srcRef = $srcCols[$i] . $r;
            $dstRef = $dstCols[$i] . $r;

            // src cell 찾기 (self-closing 포함)
            $srcRe = '/<c\b([^>]*)\br="' . preg_quote($srcRef, '/') . '"([^>]*)\/>|<c\b([^>]*)\br="' . preg_quote($srcRef, '/') . '"([^>]*)>(.*?)<\/c>/s';
            if (!preg_match($srcRe, $rowInner, $sm)) continue;

            // content 추출
            $srcAttrs = '';
            $srcContent = '';
            if (!empty($sm[5]) || $sm[5] === '0') {
                // normal <c>...</c>
                $srcAttrs = (string)($sm[3] . $sm[4]);
                $srcContent = (string)$sm[5];
            } else {
                // self-closing <c .../>
                $srcAttrs = (string)($sm[1] . $sm[2]);
                $srcContent = '';
            }

            // 값이 없으면(빈 셀) 복사할 필요 없음
            if ($srcContent === '' || trim($srcContent) === '') continue;

            $srcT = null;
            if (preg_match('/\bt="([^"]+)"/', $srcAttrs, $tm)) $srcT = $tm[1];

            // dst cell이 있으면 content를 교체, 없으면 새로 생성(스타일 없음)
            $dstReSelf = '/<c\b([^>]*)\br="' . preg_quote($dstRef, '/') . '"([^>]*)\/>/s';
            $dstReNorm = '/<c\b([^>]*)\br="' . preg_quote($dstRef, '/') . '"([^>]*)>(.*?)<\/c>/s';

            if (preg_match($dstReSelf, $rowInner, $dm)) {
                $dstAttrs = (string)($dm[1] . $dm[2]);

                // t 속성은 src에 맞추고, style(s)은 dst 유지
                if ($srcT !== null) {
                    if (preg_match('/\bt="[^"]+"/', $dstAttrs)) {
                        $dstAttrs = preg_replace('/\bt="[^"]+"/', 't="' . $srcT . '"', $dstAttrs);
                    } else {
                        $dstAttrs .= ' t="' . $srcT . '"';
                    }
                } else {
                    $dstAttrs = preg_replace('/\s+t="[^"]+"/', '', $dstAttrs);
                }

                $newCell = '<c ' . trim($dstAttrs) . '>' . $srcContent . '</c>';
                $rowInner = preg_replace($dstReSelf, $newCell, $rowInner, 1);
                $copied++;
                continue;
            }

            if (preg_match($dstReNorm, $rowInner, $dm)) {
                $dstAttrs = (string)($dm[1] . $dm[2]);
                ...
                $newCell = '<c ' . trim($dstAttrs) . '>' . $srcContent . '</c>';
                $rowInner = preg_replace($dstReNorm, $newCell, $rowInner, 1);
                $copied++;
                continue;
            }

            // dst cell이 아예 없으면 새로 삽입 (스타일 없음)
            $newCell = '<c r="' . $dstRef . '"' . ($srcT !== null ? ' t="' . $srcT . '"' : '') . '>' . $srcContent . '</c>';
            $rowInner .= $newCell;
            $copied++;
        }

        return '<row' . $rowAttr . '>' . $rowInner . '</row>';
    }, $sheetXml);

    if ($sheetXml2 === null) {
        return ['xml' => $sheetXml, 'copied' => 0];
    }
    return ['xml' => $sheetXml2, 'copied' => $copied];
}

function report_apply_fai_spc_colcfg_to_xlsx(string $xlsxPath, string $partName, bool $isJawha, bool $debug = false): array {
    $cfg = $GLOBALS['REPORT_FAI_SPC_COLCFG'] ?? null;
    ...
    $src = $cfg['src'] ?? ['FAI' => 'AK', 'SPC' => 'AK'];
    $dstMap = $cfg['dst'] ?? [];
    $dst = $dstMap[$vendorKey] ?? ['FAI' => ($src['FAI'] ?? 'AK'), 'SPC' => ($src['SPC'] ?? 'AK')];

    $byModel = $cfg['by_model'][$partName] ?? null;
    if (is_array($byModel)) {
        if (isset($byModel['src']) && is_array($byModel['src'])) {
            $src = array_merge($src, $byModel['src']);
        }
        if (isset($byModel['dst']) && is_array($byModel['dst'])) {
            $dst = array_merge($dst, $byModel['dst'][$vendorKey] ?? []);
        }
    }

    $srcFai = strtoupper(trim((string)($src['FAI'] ?? '')));
    $srcSpc = strtoupper(trim((string)($src['SPC'] ?? '')));
    $dstFai = strtoupper(trim((string)($dst['FAI'] ?? '')));
    $dstSpc = strtoupper(trim((string)($dst['SPC'] ?? '')));

    // 아무 변화 없으면 스킵
    $todo = [];
    if ($srcFai !== '' && $dstFai !== '' && $srcFai !== $dstFai) $todo[] = ['kw' => 'FAI', 'src' => $srcFai, 'dst' => $dstFai];
    if ($srcSpc !== '' && $dstSpc !== '' && $srcSpc !== $dstSpc) $todo[] = ['kw' => 'SPC', 'src' => $srcSpc, 'dst' => $dstSpc];
    if (!$todo) return ['ok' => false, 'reason' => 'no_change'];

    $zip = new ZipArchive();
    if ($zip->open($xlsxPath) !== true) {
        return ['ok' => false, 'reason' => 'zip_open_failed'];
    }
    ...
    foreach ($todo as $t) {
        $kw = $t['kw'];
        $xmlPath = report_xlsx_find_sheet_xml_by_keyword($zip, $kw);
        if (!$xmlPath) {
            $missingSheets[] = $kw;
            continue;
        }
        ...
        // FAI는 샘플 3칸(L/M/N 또는 H/I/J 등), SPC는 측정 32칸(AM..BR 또는 AI..BN 등)
        $width = ($kw === 'FAI') ? 3 : 32;
        $r = report_xlsx_copy_toolcols_in_sheet_xml($xml, $t['src'], $t['dst'], $width);
        if (!empty($r['copied'])) {
            $zip->addFromString($xmlPath, $r['xml']);
            $totalCopied += (int)$r['copied'];
            $applied++;
        }
    }
    ...
    if ($debug && $totalCopied > 0) {
        logline("  - REPORT 열 교정 적용: {$partName} / {$vendorKey} / copied={$totalCopied}");
        if ($missingSheets) logline("  - REPORT 시트 미발견: " . implode(',', $missingSheets));
    }
...
// (REPORT) FAI/SPC 시작열 교정(옵션)
if (!empty($GLOBALS['REPORT_FAI_SPC_COLCFG']['enabled'])) {
    try {
        report_apply_fai_spc_colcfg_to_xlsx(
            $createdReportByPart[$part],
            $part,
            (bool)($IS_JAWHA ?? false),
        ...
        if (!empty($GLOBALS['DEBUG'])) logline("  - REPORT 열 교정 실패: " . $e3->getMessage());
    }
}
            } catch (Throwable $e2) {
...
?></div>
  ...
  <div class="card small">
    ...
    <a style="color:#8ab4f8;" href="?action=fetch&token=<?=h($token)?>">ZIP 다운로드</a>
  </div>
</div>
...
setTimeout(() => {
  location.href = '?action=fetch&token=<?=h($token)?>';
}, 800);
</script>
</body>
</html>
