...
// 파일선택 성적서 미리보기: 현재 성적서 페이지에서 직접 처리한다.
// 라우터/iframe URL에서도 상대경로 JS/PHP 의존 없이 동작하도록 self endpoint로 고정.
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && (string)$action === 'file_report_preview') {
    header('Content-Type: application/json; charset=utf-8');

    $sfrJsonError = static function(string $message, int $status = 400): void {
        http_response_code($status);
        echo json_encode(['ok' => false, 'message' => $message], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    };

    if (!$__shipReportCanBuild) {
        $sfrJsonError('권한이 없습니다. lv 2 이상만 성적서 생성이 가능합니다.', 403);
    }
    if (!isset($_FILES['shipping_file']) || !is_array($_FILES['shipping_file'])) {
        $sfrJsonError('출하 엑셀 파일을 선택해 주세요.');
    }

    $f = $_FILES['shipping_file'];
    if ((int)($f['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        $sfrJsonError('파일 업로드에 실패했습니다.');
    }

    $filename = (string)($f['name'] ?? '');
    $ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
    if (!in_array($ext, ['xls', 'xlsx'], true)) {
        $sfrJsonError('xls 또는 xlsx 파일만 선택할 수 있습니다.');
    }

    $tmp = (string)($f['tmp_name'] ?? '');
    if ($tmp === '' || !is_uploaded_file($tmp)) {
        $sfrJsonError('업로드 파일을 확인할 수 없습니다.');
    }

    $fromDatePreview = trim((string)($_POST['from_date'] ?? ''));
    $toDatePreview   = trim((string)($_POST['to_date'] ?? ''));
    $shipToPreview   = trim((string)($_POST['ship_to'] ?? ''));

    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $fromDatePreview) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $toDatePreview)) {
        $sfrJsonError('조회기간을 확인해 주세요.');
    }

    $detected = sfr_detect_ship_to_from_filename($filename);
    if ($detected !== '') $shipToPreview = $detected;
    if ($shipToPreview === '') $sfrJsonError('납품처를 선택해 주세요.');

    try {
        $sfrReader = IOFactory::createReaderForFile($tmp);
        if (method_exists($sfrReader, 'setReadDataOnly')) $sfrReader->setReadDataOnly(true);
        $sfrBook = $sfrReader->load($tmp);
    } catch (Throwable $e) {
        $sfrJsonError('엑셀 파일을 읽을 수 없습니다.', 400);
    }

    $headerFound = false;
    $sheetCandidates = [];
    try {
        foreach ($sfrBook->getWorksheetIterator() as $sfrSheet) {
            $matrix = $sfrSheet->toArray(null, true, true, false);
            $parsed = sfr_extract_sheet_rows($matrix, 30);
            if (empty($parsed['header_found'])) continue;
            $headerFound = true;
            $sheetCandidates[] = [
                'sheet' => $sfrSheet->getTitle(),
                'rows' => (array)($parsed['rows'] ?? []),
                'headers' => (array)($parsed['headers'] ?? []),
            ];
        }
    } finally {
        if (isset($sfrBook) && method_exists($sfrBook, 'disconnectWorksheets')) $sfrBook->disconnectWorksheets();
        unset($sfrBook, $sfrReader);
    }

    if (!$headerFound) {
        $sfrJsonError('엑셀에서 「포장번호」 컬럼을 찾지 못했습니다.');
    }
    ...
    if (!$fileRows) {
        $sfrJsonError('「포장번호」 컬럼에 출하 데이터가 없습니다.');
    }
    if (($headers['qty'] ?? null) === null) {
        $sfrJsonError('포장번호가 있는 시트에서 출하수량 컬럼(총납품수량/출고수량/출하수량)을 찾지 못했습니다.');
    }
    ...
    if (!$packNos) $sfrJsonError('유효한 포장번호가 없습니다.');

    $cutoff = '08:30:00';
    $fromTsPreview = strtotime($fromDatePreview . ' ' . $cutoff . ' -1 day');
    $toTsPreview   = strtotime($toDatePreview   . ' ' . $cutoff);
    if ($fromTsPreview === false || $toTsPreview === false || $fromTsPreview >= $toTsPreview) {
        $sfrJsonError('조회기간이 올바르지 않습니다.');
    }
    $fromDtPreview = date('Y-m-d H:i:s', $fromTsPreview);
    $toDtPreview   = date('Y-m-d H:i:s', $toTsPreview);
    ...
    try {
        foreach (array_chunk($packNos, 300) as $chunkIndex => $chunk) {
            ...
            $sqlPreview = "
                ...
                ORDER BY id
            ";
            $paramsPreview = array_merge([
                ':from_dt' => $fromDtPreview,
                ':to_dt' => $toDtPreview,
                ':ship_to' => $shipToPreview,
            ], $packParams);
            $stPreview = $pdo->prepare($sqlPreview);
            $stPreview->execute($paramsPreview);
            while ($r = $stPreview->fetch(PDO::FETCH_ASSOC)) $dbRows[] = $r;
        }
    } catch (Throwable $e) {
        $sfrJsonError('출하 DB 대조 중 오류가 발생했습니다.', 500);
    }
    ...
    if ($canConfirm) {
        try {
            $selectionToken = sfr_selection_save([
                'pack_nos' => (array)($preview['matched_pack_nos'] ?? $packNos),
                'ship_to' => $shipToPreview,
                'from_date' => $fromDatePreview,
                'to_date' => $toDatePreview,
                'filename' => $filename,
            ]);
        } catch (Throwable $e) {
            $sfrJsonError('파일선택 정보를 저장할 수 없습니다.', 500);
        }
    }
