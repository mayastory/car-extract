...
function jtgpt_shipping_message_export_scope_hint(string $message): string {
...
function jtgpt_shipping_export_scope(array $args): string {
    $scope = strtolower(trim((string)($args['export_scope'] ?? '')));
    if (in_array($scope, ['summary', 'detail'], true)) {
        return $scope;
    }
    return 'summary';
}

function jtgpt_shipping_summary_export_rows(array $args, array $result): array {
    $scopeText = jtgpt_format_scope($args);
    $rows = [];
    if (!empty($result['row']) && is_array($result['row'])) {
        ...
        $rows[] = [
            ...
            '유형' => '최근출하',
            '항목' => '일시',
            '값' => (string)($row['ship_datetime'] ?? ''),
            '품번' => (string)($row['part_name'] ?? ''),
            '납품처' => (string)($row['ship_to'] ?? ''),
            '수량(EA)' => jtgpt_tool_format_int($row['qty'] ?? 0),
        ];
        return $rows;
    }

    $rows[] = ['범위' => $scopeText, '유형' => '요약', '항목' => '총 수량', '값' => jtgpt_tool_format_int($result['total_qty'] ?? 0) . ' EA', '품번' => '', '납품처' => '', '수량(EA)' => ''];
    $rows[] = ['범위' => $scopeText, '유형' => '요약', '항목' => 'LOT 수', '값' => jtgpt_tool_format_int($result['lot_count'] ?? 0), '품번' => '', '납품처' => '', '수량(EA)' => ''];
    $rows[] = ['범위' => $scopeText, '유형' => '요약', '항목' => 'Tray 수', '값' => jtgpt_tool_format_int($result['tray_count'] ?? 0), '품번' => '', '납품처' => '', '수량(EA)' => ''];
    foreach ((array)($result['top_parts'] ?? []) as $partRow) {
        $rows[] = [
            '범위' => $scopeText,
            '유형' => '상위품번',
            '항목' => (string)($partRow['part_name'] ?? ''),
            '값' => jtgpt_tool_format_int($partRow['total_qty'] ?? 0) . ' EA',
            '품번' => (string)($partRow['part_name'] ?? ''),
            '납품처' => '',
            '수량(EA)' => jtgpt_tool_format_int($partRow['total_qty'] ?? 0),
        ];
    }
    return $rows;
}
...
function jtgpt_shipping_create_export(PDO $pdo, array $args, ?array $result = null): ?array {
    $output = jtgpt_quality_output_format($args);
    if (!in_array($output, ['excel', 'csv'], true)) {
        return null;
    }

    $scope = jtgpt_shipping_export_scope($args);
    if ($scope === 'detail') {
        $rows = jtgpt_shipping_export_source_rows($pdo, $args);
    } else {
        $summaryResult = is_array($result) ? $result : [];
        if (!$summaryResult) {
            $queryArgs = $args;
            unset($queryArgs['output']);
            $summaryResult = !empty($queryArgs['metric'])
                ? jtgpt_tool_shipping_summary($pdo, $queryArgs)
                : jtgpt_tool_shipping_last_ship_date($pdo, $queryArgs);
        }
        $rows = jtgpt_shipping_summary_export_rows($args, $summaryResult);
    }
    if (!$rows) {
        return null;
    }
...
function jtgpt_shipping_export_brief_text(array $args, array $download, bool $followup = false): string {
    ...
    $exportScope = strtolower(trim((string)($download['export_scope'] ?? $args['export_scope'] ?? 'summary')));
    $targetLabel = $exportScope === 'detail' ? '출하 상세내역' : '출하 요약';
    if ($followup) {
        return $count !== null
            ? ('방금 조회한 ' . $targetLabel . ' ' . jtgpt_tool_format_int($count) . '행을 ' . $label . ' 파일로 만들었습니다.')
            : ('방금 조회한 ' . $targetLabel . '을 ' . $label . ' 파일로 만들었습니다.');
    }
    $prefix = trim(($scopeText !== '' ? ($scopeText . ' ') : '') . $targetLabel);
    return $count !== null
        ? ($prefix . ' ' . jtgpt_tool_format_int($count) . '행을 ' . $label . ' 파일로 만들었습니다.')
...
function jtgpt_build_export_followup_plan_from_tool_args(string $tool, array $args, string $message, string $source): ?array {
    ...
    if (strpos($tool, 'shipping_') === 0) {
        $hintScope = jtgpt_shipping_message_export_scope_hint($message);
        if ($hintScope !== '') {
            $args['export_scope'] = $hintScope;
        } elseif (empty($args['export_scope'])) {
            $args['export_scope'] = 'summary';
        }
    }
    return [
        'kind' => 'tool',
    ...
    if (($plan['kind'] ?? '') === 'clarify') {
        ...
        if (!$pdo instanceof PDO) {
            ...
            if (isset($res) && is_array($res) && jtgpt_is_exportable_tool_name($tool)) {
                ...
                if (strpos($tool, 'shipping_') === 0 && empty($baseArgs['export_scope'])) {
                    $baseArgs['export_scope'] = 'summary';
                }
...
" . $ttlText;
                        ...
                        $download = jtgpt_shipping_create_export($pdo, $args, isset($res) && is_array($res) ? $res : null);
                        if ($download) {
                            $ttlText = !empty($download['expires_in_sec']) ? ('다운로드 후 약 ' . max(1, (int)round(((int)$download['expires_in_sec']) / 60)) . '분 뒤 자동 정리됩니다.') : '';
