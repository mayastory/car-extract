...
if ($view === '') {
 ...
 elseif (preg_match('#/qr_reader/?(\?|$)#', $uri)) $view = 'qr_reader';
...
// 매트릭스 설정 로드(한 곳에서 관리)
$mb = @include JTMES_ROOT . '/config/matrix_bg.php';
...
$matrixJsV = @filemtime(JTMES_ROOT . '/assets/matrix-bg.js');

if (!$matrixJsV) $matrixJsV = time();

$shellJsV = @filemtime(JTMES_ROOT . '/assets/dp_shell.js');

$authWatchV = @filemtime(JTMES_ROOT . '/assets/dp_auth_watchdog.js');

if (!$authWatchV) $authWatchV = time();
if (!$shellJsV) $shellJsV = time();

$unifiedCssV = @filemtime(JTMES_ROOT . '/assets/dp_theme_unified.css');
...
<head>
 ...
 <script defer src="assets/dp_auth_watchdog.js?v=<?php echo (int)$authWatchV; ?>"></script>
...
<div class="dp-shell-wrap">
 ...
 <iframe id="dpShellFrame" src="<?php echo h($iframeSrc); ?>" loading="eager" referrerpolicy="no-referrer" allow="camera; microphone; fullscreen"></iframe>
