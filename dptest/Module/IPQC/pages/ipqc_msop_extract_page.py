# -*- coding: utf-8 -*-
"""Extract one MSOP FAI page from a PDF.

Args:
  1: source pdf path
  2: fai label, e.g. "FAI 1 / SPC A", "14-P2"
  3: output pdf path
...
def _find_page(reader, fai_label: str):
...
def main():
    ...
    try:
        ...
        page_idx, err = _find_page(reader, fai_label)
        if page_idx is None:
            _emit(ok=False, error=err or "FAI page not found")
            return 1
        ...
        _emit(ok=True, page=page_idx + 1, out=out_path)
        return 0
    except Exception as e:
