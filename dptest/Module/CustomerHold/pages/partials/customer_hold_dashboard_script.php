...
function renderDashboard(){
    ...
    allRows.forEach(function(row){
        ...
        if (status.code === 'blocked') blockedRows.push(row); else availableRows.push(row);
        if (!dashboardVendorMatches(row, vendorFilter)) return;
        if (statusFilter !== 'all' && status.code !== statusFilter) return;
    ...
    if (!selectedEntry && visibleRows.length) {
        ...
        selectedEntry = visibleRows.find(function(row){ return dashboardEntryKey(row) === state.dashboardSelectedKey; }) || visibleRows[0];
    }
