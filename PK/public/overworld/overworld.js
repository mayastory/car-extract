import {
   getActorPriorityState, getFrontCoverAt as getOwFrontCoverAt, getFrontOccluderAt as getOwFrontOccluderAt, getGrassCoverAt as getOwGrassCoverAt, isTallGrassBehavior as isOwTallGrassBehavior }
 from "./overworld_priority.js";

export class Overworld{
  
  constructor({
    canvas,status,playToken=null,apiBase="/api", fixedZoom=2.0, lockZoom=true}
  ){
    ...
    this.playerSprite={
       kind:"frlg9", frameW:16, frameH:32, framesPerDir:3 }
    ;
    ...
    this.moveFramesPerTile = 16;
                     // 16 frames -> 0.2666s per tile
    this.moveSeconds = this.moveFramesPerTile / this.fixedFps;
     // (legacy) seconds per tile
    this.stepCooldown = 0.0;
                         // seconds AFTER each step (set e.g. 0.04 if you want a tiny pause)
    ...
    this.player={
      x:0,y:0,dir:0,px:0,py:0,moving:false}
    ;
    ...
    this._mobIconFrameH=32;
     // icons are usually 32x64 (2 frames stacked)
    ...
    this._edgePending=null;
                   // set while stepping across a connected border
    this._moveSecondsNow=null;
                // per-move override (e.g., jump)
    this.jumpSeconds=0.38;
    ...
    this._dbg={
       enabled:dbgOn, max:300, lines:[], root:null, body:null, visible:dbgOn }
    ;
  ...
  _dbgRender(){
    ...
    try{
       this._dbg.body.scrollTop=this._dbg.body.scrollHeight;
       }
    catch(_e){
      }
  ...
  _publicUrl(p){
    ...
    const rel=String(p).replace(/^\//,'');
     // treat leading '/' as relative-to-public
  ...
  img.onerror=()=>{
       this._mobIconErr.add(cacheKey);
       this._mobIconImg.delete(cacheKey);
       }
    ;
  ...
  _mobPos(m){
    ...
    if(!s){
      ...
      return {
         x:(m.x|0), y:(m.y|0), moving:false, flipX:false, dir:2, bob:0, seed }
      ;
      
    }
    
    return {
       x:s.x, y:s.y, moving:!!s.moving, flipX:!!s.flipX, dir:(s.dir|0), bob:(s.bob|0), seed:(s.seed|0) }
    ;
...
// Player sprite: fallback to placeholder if red_normal is missing.
  ...
  async _resyncFromServerState(){
    ...
    try{
      ...
      const s = await fetch(`${this.apiBase}/rt/get.php`, {
  ...
  async _initFromServer(){
    ...
    try{
      ...
      const w = await fetch(`${this.apiBase}/auth/whoami.php`, {
        cache:"no-store", headers: hdr}
      );
  ...
  _groundAt(x,y){
    ...
    // Only connected overworld maps should extend via border metatiles.
    // Interiors and stand-alone maps must stay black outside bounds; otherwise
    // doorway/F5 reloads smear the previous town border into the lab/house view.
    const hasConnections = Array.isArray(this.map?.connections) && this.map.connections.some(c=>c && c.map_id);
  ...
  _trySeamlessEdgeMove(nx,ny,dx,dy){
    ...
    if(tx<0||ty<0||tx>=nb.map.width||ty>=nb.map.height) return;
     // invalid => block
    ...
    this._edgePending={
       dir, map_id:c.map_id, offset:off, fromW:W, fromH:H }
    ;
  ...
  _openDialog(pages){
    ...
    if(this.keys && this.keys.clear) this.keys.clear();
     // prevent walking while dialog opens
    this._dialog = {
       open: true, pages: ps, idx: 0, char: 0, _t: 0 }
    ;
  ...
  _drawTextWindow9(oc, img, x, y, w, h){
    ...
    for(let xx=x0+t;
     ...
     xx+=t){
      ...
      oc.drawImage(img, t,0,t,t, xx,y0,t,t);
             // top
      oc.drawImage(img, t,2*t,t,t, xx,y1,t,t);
           // bottom
    }
    ...
    for(let yy=y0+t;
     ...
     yy+=t){
      ...
      oc.drawImage(img, 0,t,t,t, x0,yy,t,t);
             // left
      oc.drawImage(img, 2*t,t,t,t, x1,yy,t,t);
           // right
    }
     ...
     yy+=t){
      ...
      for(let xx=x0+t;
       ...
       xx+=t){
        ...
        oc.drawImage(img, t,t,t,t, xx,yy,t,t);
             // center
      }
  ...
  _tileInfoAt(mx,my){
    ...
    if(ctx){
      ...
      return {
         tile:t, img, cols: assets.tilesetCols || this.tilesetCols || 16 }
      ;
    ...
    // Non-connected out-of-bounds tiles must stay as border/0.
    // Clamping to the nearest in-map tile makes interiors look artificially extended
    // and smears edge tiles into the black area. Connected maps are already handled
    // above by _resolveMapContextAt().
    return {
       tile:t, img:this._currentTilesetImg(), cols:this.tilesetCols||16 }
    ;
  ...
  _tileUpperInfoAt(mx,my){
    ...
    // Never clamp upper-layer tiles outside the current map. Clamping smears wall/
    // shelf/window overlays from the last valid interior tile across the black void.
    return null;
...
// Walking animation
  ...
  _draw(){
    ...
    const queueRowDraw = (row, sortX, drawFn, cover=null, southCover=null) => {
      ...
      bucket.push({
         x: +sortX || 0, draw: drawFn, cover, southCover }
      );
...
async _ping(){
    ...
    try{
      ...
      const r=await fetch(`${this.apiBase}/util/health.php`,{
        cache:"no-store"}
      );
